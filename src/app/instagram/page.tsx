...
export default function InstagramPage() {
  ...
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      ...
      if (detected !== "instagram" && detected !== "unknown") {
        clearPreview();
        setErrorMessage(
          `This looks like a ${detected} link. Please paste an Instagram URL here or use the ${detected} downloader.`,
        );
        return;
      }
    ...
    <main className="page-gradient min-h-screen px-4 pt-24">
      ...
      <div
        ...
        <p className="card-muted mb-6">
          {activeTab === "photo"
            ? "Paste an Instagram image URL and download it instantly."
            : activeTab === "audio"
              ? "Extract high-quality audio from Instagram videos instantly."
              : "Paste an Instagram video URL and download it in your preferred quality."}
        </p>
        ...
        <input
          ...
          placeholder={
            activeTab === "photo"
              ? "Paste Instagram image URL here..."
