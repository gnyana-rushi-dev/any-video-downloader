...
export default function TwitterPage() {
  ...
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      ...
      if (detected !== "twitter" && detected !== "unknown") {
        clearPreview();
        setErrorMessage(
          `This looks like a ${detected} link. Please paste a Twitter/X URL here or use the ${detected} downloader.`,
        );
        return;
      }
      fetchPreview(inputValue, "twitter", activeTab);
    }, 800);
    ...
    <main className="page-gradient min-h-screen px-4 pt-24">
      ...
      <div
        ...
        <p className="card-muted mb-6">
          {activeTab === "photo"
            ? "Paste a Twitter image URL and download it instantly."
            : activeTab === "audio"
              ? "Extract high-quality audio from Twitter videos instantly."
              : "Paste a Twitter video URL and download it in your preferred quality."}
        </p>
        ...
        <input
          ...
          placeholder={
            activeTab === "photo"
              ? "Paste Twitter image URL here..."
              : "Paste Twitter video URL here..."
          }
